// Copyright (c) 2016-present Flywave, Inc. All Rights Reserved.
...
var I18nCmd = &cobra.Command{
	Use:   "i18n",
	Short: "Management of Flywave translations",
}
...
func init() {
	ExtractCmd.Flags().String("enterprise-dir", "../enterprise", "Path to folder with the Flywave enterprise source code")
	ExtractCmd.Flags().String("flywave-dir", "./", "Path to folder with the Flywave source code")
	CheckCmd.Flags().String("enterprise-dir", "../enterprise", "Path to folder with the Flywave enterprise source code")
	CheckCmd.Flags().String("flywave-dir", "./", "Path to folder with the Flywave source code")
	I18nCmd.AddCommand(
		ExtractCmd,
		CheckCmd,
	)
...
func getCurrentTranslations(flywaveDir string) ([]Translation, error) {
	jsonFile, err := ioutil.ReadFile(path.Join(flywaveDir, "i18n", "en.json"))
	if err != nil {
		return nil, err
	}
...
func extractStrings(enterpriseDir, flywaveDir string) map[string]bool {
	i18nStrings := map[string]bool{}
	walkFunc := func(p string, info os.FileInfo, err error) error {
		if strings.HasPrefix(p, path.Join(flywaveDir, "vendor")) {
			return nil
		}
		return extractFromPath(p, info, err, &i18nStrings)
	}
	filepath.Walk(flywaveDir, walkFunc)
...
func extractCmdF(command *cobra.Command, args []string) error {
	...
	flywaveDir, err := command.Flags().GetString("flywave-dir")
	if err != nil {
		return errors.New("Invalid flywave-dir parameter")
	}

	i18nStrings := extractStrings(enterpriseDir, flywaveDir)
	...
	translations, err := getCurrentTranslations(flywaveDir)
	if err != nil {
		return err
	}
	...
	f, err := os.Create(path.Join(flywaveDir, "i18n", "en.json"))
	defer f.Close()
